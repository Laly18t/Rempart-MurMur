...
export function ScrollControls({
  ...
  }, [el, events, size, infinite, state, invalidate, horizontal, enabled, scrollFactor])
...
const ScrollCanvas = /* @__PURE__ */ React.forwardRef(
  ({ children }: ScrollProps, ref: React.ForwardedRef<THREE.Group>) => {
    ...
    // Accéder au scrollFactor à partir du context state
    const scrollFactor = state.scrollFactor || 1

    useFrame(() => {
      const adjustedOffset = state.offset / scrollFactor
      group.current.position.x = state.horizontal ? -width * (state.pages - 1) * adjustedOffset : 0
      group.current.position.y = state.horizontal ? 0 : height * (state.pages - 1) * adjustedOffset
    })
